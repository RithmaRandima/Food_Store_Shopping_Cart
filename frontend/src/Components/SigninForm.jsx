...
const SigninForm = ({ switchToSignin, popup, setPopup }) => {
  ...
  return (
    <>
      ...
      <div className="hidden md:flex w-[800px] h-[500px] shadow-[1px_1px_5px_rgba(0,0,0,0.9)] bg-white relative">
        {/* loading window */}
        ...
        <div className="flex-1 flex flex-col relative">
          ...
          <form onSubmit={handelSubmit}>
            ...
            <div>
              <button
                className="mb-8 mt-5 bg-[#6a9c06] text-[18px] text-white p-1 pr-10 -ml-3 shadow-[1px_1px_5px_rgba(0,0,0,0.9)] font-extrabold"
                type="submit"
              >
                Sign in
              </button>
      ...
      <div className="flex md:hidden flex-col w-full min-h-screen bg-white px-5 py-8">
        {/* title */}
        <h1 className="text-[#6a9c06] text-3xl font-extrabold mb-6">Sign In</h1>

        <form onSubmit={handelSubmit} className="flex flex-col gap-5">
          {/* email */}
          <div>
            <label className="block text-[#6a9c06] font-bold">Email</label>
            <input
              name="email"
              type="email"
              placeholder="Enter Email"
              value={user.email}
              className="w-full mt-2 px-3 py-2 border border-[#6a9c06] text-black"
              onChange={(e) => {
                setUser({ ...user, [e.target.name]: e.target.value });
                setPopup({ ...popup, show: false });
              }}
          ...
          {/* password */}
          <div>
            <label className="block text-[#6a9c06] font-bold">Password</label>
            <input
              name="password"
              type="password"
              placeholder="Enter Password"
